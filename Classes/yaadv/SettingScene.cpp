...
bool SettingScene::init() {
  ...
  auto skipallTxtPh = Sprite::create(vars["yaadv/ui/config/txt_skipall.jpg"]);
  skipallTxtPh->setPosition(Vec2(840 + origin.x, 212 + origin.y));
  stageLayer->addChild(skipallTxtPh);

  _textSpeedSlb =
      rx::Slidebar::createSlidebar(vars["yaadv/ui/config/slb_background.png"], vars["yaadv/ui/config/slb_ball.png"]);
  ...
  _soundVolumeSlidebar->moveEvent = CC_CALLBACK_0(SettingScene::changeSoundVolume, this);
  _soundVolumeSlidebar->touchEvent = CC_CALLBACK_0(SettingScene::changeSoundVolume, this);
  stageLayer->addChild(_soundVolumeSlidebar);

  auto ebSkipReadOnly = EasyButton::createEasyButton(vars["yaadv/ui/charbox_off.png"], vars["yaadv/ui/charbox_off.png"],
                                                     vars["yaadv/ui/config/yes.png"]);
  ebSkipReadOnly->setPosition(Vec2(740, 212));

  auto ebSkipAll = EasyButton::createEasyButton(vars["yaadv/ui/charbox_off.png"], vars["yaadv/ui/charbox_off.png"],
                                                vars["yaadv/ui/config/yes.png"]);
  ebSkipAll->setPosition(Vec2(940, 212));

  _isSkipAll = rx::RadioButton::createRadioButton(ebSkipReadOnly, ebSkipAll, nullptr);
  ...
  auto buttonBack =
      MenuItemImage::create(vars["yaadv/ui/config/btn_back_normal.png"], vars["yaadv/ui/config/btn_back_touch.png"],
...
void SettingScene::changeMusicVolume() {
  ...
  AudioHelper::getAudioEngine()->setBackgroundMusicVolume(GameSystem::getInstance()->getMusicVolume());
}

void SettingScene::changeSoundVolume() {
  GameSystem::getInstance()->setSoundVolume(_soundVolumeSlidebar->getFloat());
  AudioHelper::getAudioEngine()->setEffectsVolume(GameSystem::getInstance()->getSoundVolume());
}

void SettingScene::changeTextSpeed() { GameSystem::getInstance()->setTextSpeed(_textSpeedSlb->getFloat()); }

void SettingScene::changeAutoSpeed() {
  GameSystem::getInstance()->setAutoSpeed(5.01f - _autoSpeedSlidebar->getFloat() * 5);
}
