...
WrenHandle* wrenCopyHandle(WrenVM* vm, WrenHandle* handle) {
  WrenHandle* c = wrenMakeHandle(vm, handle->value);
}
...
WrenInterpretResult wrenInterpretExpr(WrenVM* vm, const char* module,
                                  const char* source) {
  ObjClosure* closure = wrenCompileSource(vm, module, source, true, true);
  if (closure == NULL) return WREN_RESULT_COMPILE_ERROR;
